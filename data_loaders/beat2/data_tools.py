...
joints_indexs_names = {}
for i, n in enumerate(joints_list["beat_smplx_joints"]):
    joints_indexs_names[i] = n
...
class FIDCalculator(object):
    ...
    def load(self, path, load_type, save_pos=False):
        ...
        if load_type == "gt":
            ...
            print("error, select gt or op for load_type")
    ...
    def calculate_fid(self, cal_type, joint_type, high_level_opt):
        ...
        if joint_type == "full_upper_body":
            ...
            print("error, plz select correct joint type")
...
def load_partial_state_dict(model, checkpoint_state_dict):
    """
    Load only the matching shape layers from a checkpoint state dictionary into the model.

    Args:
        model (nn.Module): The model to load the state dictionary into.
        state_dict_path (str): Path to the checkpoint state dictionary.
    ...
    for name, param in checkpoint_state_dict.items():
        if name in model_state_dict:
            if param.shape == model_state_dict[name].shape:
                ...
                print(f"Shape mismatch for layer {name}: checkpoint {param.shape}, model {model_state_dict[name].shape}")
