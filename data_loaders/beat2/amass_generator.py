...
amass_test_split = ["MoSh"]
amass_vald_split = ["MoSh"]
amass_train_split = [
    "BioMotionLab_NTroje",
    "Eyes_Japan_Dataset",
    "TotalCapture",
...
amass_splits = {"test": amass_test_split, "val": amass_vald_split, "train": amass_train_split}
...
class AMASS2CacheGenerator:
    ...
    def build_cache(self, cache_folder_path: str, data_folder: str, split: str, force_build: bool = False):
        ...
        if build_cache:
            ...
            if split == "test":
                self._cache_generation(cache_folder_path, split, data_folder, True, 0, 0, is_test=True)
            else:
    ...
    def _cache_generation(
        ...
        clean_final_seconds,
        is_test=False,
        amass_dir="./datasets/AMASS-SMPLX/",
    ):
        ...
        for dataset in amass_splits[split]:
            ...
            for index, file_name in enumerate(npz_files):
                try:
                    ...
                    trans_v_each_file = []
                    shape_each_file = []
                    audio_each_file = None
                    facial_each_file = []
                    word_each_file = []
                    emo_each_file = []
                    sem_each_file = []
                    vid_each_file = []
                    audio_each_file_resample = None
                    ...
                    if "smplx" in self.args.pose_rep:
                        ...
                        # Poses is size of 165:
                        # global_orient - [:3]
                        # body_pose = [3:]

                        n, c = poses.shape[0], poses.shape[1]  # Motion Length, pose_dim
                        betas = betas.reshape(1, 16)  # number of betas is 300
                        ...
                        exps = torch.from_numpy(np.zeros((n, 0))).to(self.device).float()
                    ...
                    if self.args.id_rep is not None:
                        vid_each_file = np.repeat(np.array(-1).reshape(1, 1), pose_each_file.shape[0], axis=0)
    ...
    def _calculate_mean_velocity(self, cache_folder_path, save_path):
        smplx_model = self.get_smplx()
