...
import smplx
import copy
from data_loaders.beat2.models.motion_encoder import *
...
class VAESKConv(VAEConv):
    def __init__(self, args):
        ...
        smpl_fname = (
            args.data_path_1 + "smplx_models/smplx/SMPLX_NEUTRAL_2020.npz"
...
class VAEConvMLP(VAEConv):
    def __init__(self, args):
        ...
        self.encoder = PoseEncoderConv(
            args.vae_test_len, args.vae_test_dim, feature_length=args.vae_length
        )
        self.decoder = PoseDecoderConv(
            args.vae_test_len, args.vae_test_dim, feature_length=args.vae_length
        )
...
class VQVAEConv(nn.Module):
    def __init__(self, args):
        ...
        self.quantizer = Quantizer(
            args.vae_codebook_size, args.vae_length, args.vae_quantizer_lambda
        )
        self.decoder = VQDecoderV3(args)
...
class VQVAE2(nn.Module):
    def __init__(self, args):
        ...
        self.top_encoder = VQEncoderV3(
            args_top
        )  # Adjust according to the top level's design
        self.quantize_conv_t = nn.Conv1d(
            args.vae_length + args.vae_length, args.vae_length, 1
        )
        ...
        layers = [
            ...
            nn.Conv1d(
                args.vae_length, args.vae_length, kernel_size=3, stride=1, padding=1
            ),
        ...
        self.top_decoder = VQDecoderV3(
            args_top
        )  # Adjust to handle top level features appropriately
    ...
    def forward(self, inputs):
        ...
        bottom_embedding_loss, quant_b, _, bottom_perplexity = self.bottom_quantizer(
            quant_b
        )
