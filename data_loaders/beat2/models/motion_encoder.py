...
"""
from data_loaders.beat2.models.quantizer import *
from data_loaders.beat2.models.utils.layer import ResBlock, init_weight
...
class VQDecoderV4(nn.Module):
    def __init__(self, args):
        ...
        if input_size == channels[0]:
            ...
            layers = [
                nn.Conv1d(input_size, channels[0], kernel_size=3, stride=1, padding=1)
            ]
        ...
        layers += [
            nn.Conv1d(channels[-1], channels[-1], kernel_size=3, stride=1, padding=1)
        ]
        self.main = nn.Sequential(*layers)
        self.main.apply(init_weight)
...
class PoseEncoderConvResNet(nn.Module):
    def __init__(self, length, dim, feature_length=32):
        ...
        self.conv1 = BasicBlock(
            dim, self.base, reduce_first=1, downsample=False, first_dilation=1
        )  # 34
        ...
        self.conv3 = BasicBlock(
            self.base * 2, self.base * 2, first_dilation=1, downsample=True, stride=2
        )  # 17
        self.conv4 = BasicBlock(
            self.base * 2, self.base, first_dilation=1, downsample=False
        )
...
class Decoder_TRANSFORMER(nn.Module):
    ...
    def forward(self, inputs):
        timequeries = torch.zeros(
            inputs.shape[0], self.vae_test_len, self.vae_length, device=inputs.device
        )
