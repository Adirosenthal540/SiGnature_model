...
import cv2
import data_loaders.beat2.utils.media
import data_loaders.beat2.utils.fast_render
...
def resize_motion_sequence_tensor(sequence, target_frames):
    ...
    target_time = torch.linspace(0, 1, target_frames, device=sequence.device).view(
        1, -1, 1
    )
    ...
    resized_sequence = torch.nn.functional.interpolate(
        sequence, size=target_frames, mode="linear", align_corners=True
    )
...
def adjust_speed_according_to_ratio_tensor(chunks):
    ...
    while success != 1:
        ...
        new_intervals = [
            chunks[0][new_intervals_boundaries[i] : new_intervals_boundaries[i + 1]]
            for i in range(4)
        ]
...
def synthesize_intermediate_frames_bidirectional(all_frames, x, n):
    ...
    for i in range(1, n):  # For each intermediate frame between x and x + n
        ...
        remap_x_forward, remap_y_forward = np.clip(
            forward_displacement[:, 1], 0, w - 1
        ), np.clip(forward_displacement[:, 0], 0, h - 1)
        remap_x_backward, remap_y_backward = np.clip(
            backward_displacement[:, 1], 0, w - 1
        ), np.clip(backward_displacement[:, 0], 0, h - 1)
        ...
        intermediate_frame = cv2.addWeighted(
            warped_forward, 1 - alpha, warped_backward, alpha, 0
        )
...
def warp_frame(src_frame, flow):
    ...
    warped_frame = cv2.remap(
        src_frame, x_coords, y_coords, interpolation=cv2.INTER_LINEAR
    )
...
def synthesize_intermediate_frames(all_frames, x, n):
    ...
    for i in range(1, n):  # For each intermediate frame
        ...
        intermediate_frame = warp_frame(
            all_frames[x], intermediate_flow
...
def smooth_rotvec_animations(animation1, animation2, blend_frames):
    ...
    for i in range(k1 // 3):
        ...
        q = R.from_rotvec(
            np.concatenate([animation1[0:1, i], animation2[-2:-1, i]], axis=0)
        )  # .as_quat()
...
def interpolate_sequence(quaternions):
    ...
    new_n = 2 * n
    new_quaternions = torch.zeros(
        (bs, new_n, j, 4), device=quaternions.device, dtype=quaternions.dtype
    )
...
def process_frame(i, vertices_all, vertices1_all, faces, output_dir, filenames):
    ...
    for vtx_idx, vtx in enumerate([vertices, vertices1]):
        trimesh_mesh = trimesh.Trimesh(
            vertices=vtx, faces=faces, vertex_colors=uniform_color
        )
...
class EpochTracker:
    def __init__(self, metric_names, metric_directions):
        ...
        self.loss_meters = {
            name: {state: AverageMeter(f"{name}_{state}") for state in self.states}
            for name in metric_names
        }

        self.is_higher_better = {
            name: direction for name, direction in zip(metric_names, metric_directions)
        }
    ...
    def update_values(self, name, state, epoch):
        ...
        if (
            value_avg < self.values[name][state]["best"]["value"]
...
def load_checkpoints(model, save_path, device, load_name="model"):
    states = torch.load(save_path, torch.device(device))
...
def model_complexity(model, args):
    ...
    flops, params = get_model_complexity_info(
        model,
        (args.T_GLOBAL._DIM, args.TRAIN.CROP, args.TRAIN),
        as_strings=False,
        print_per_layer_stat=False,
    )
    logging.info(
        "{:<30}  {:<8} BFlops".format("Computational complexity: ", flops / 1e9)
    )
